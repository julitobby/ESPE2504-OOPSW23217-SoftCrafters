...
public class Menu {
	...
	public User showLoginMenu() {
		...
		while (user == null) {
			...
			if (System.currentTimeMillis() < lockoutTime) {
				...
				System.out
						.println("Demasiados intentos fallidos. Por favor espera " + remainingSeconds + " segundos...");
				try {
			...
			if (option.equals("2")) {
				...
				if (user == null) {
					attempts++;
					System.out.println("Nombre de usuario o contrasena incorrectos. Intento " + attempts + " de "
							+ MAX_ATTEMPTS + ".\n");

					if (attempts >= MAX_ATTEMPTS) {
						System.out.println(
								"Has excedido el numero maximo de intentos. Espera 60 segundos para volver a intentar.\n");
	...
	public static void showPlanificationByTeacherId(String teacherId) {
		...
		if (foundPlan != null) {
			...
			// partsOfMethodologicalStrategy[1] + ", " + partsOfMethodologicalStrategy[2]);
		} else {
	...
	public static String generateRandomId() {
		...
		String randomPart = java.util.UUID.randomUUID().toString().substring(0, 6).toUpperCase(); // 6 caracteres
																									// aleatorios
